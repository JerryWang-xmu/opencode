...
describe("plugin.openai.ws-pool", () => {
  ...
  test("preserves HTTP fallback state across idle timeouts", async () => {
    ...
    // Wait longer than idleTimeout. The pool preserves fallback state for
    // entries with accumulated stream failures so the retry budget is not
    // reset by the prune timer.
    await new Promise((resolve) => setTimeout(resolve, 50))
    const second = await fetch(server.url, streamRequest())

    expect(await second.text()).toBe("http")
    // Fallback persists: no new WebSocket attempt after the first exhausted retries.
