...
export function createWebSocketFetch(options?: CreateWebSocketFetchOptions) {
  ...
  function prune() {
    ...
    for (const [key, entry] of pool) {
      ...
      // Preserve entries with accumulated stream failures. Mid-retry entries
      // (streamFailures > 0, !fallback) need their failure count for the next
      // attempt. Fallback entries must keep routing to HTTP until the session
      // succeeds and resets state. Pruning would reset streamFailures to 0
      // on the next request, breaking the retry budget.
      if (entry.streamFailures > 0) continue
